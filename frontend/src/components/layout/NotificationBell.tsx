...
const NotificationBell: React.FC = () => {
    ...
    return (
        <div className="relative" ref={dropdownRef}>
            ...
            {isOpen && (
                <div className="absolute right-0 mt-3 w-96 glass-dark rounded-[2rem] shadow-2xl border border-white/5 z-50 overflow-hidden animate-in fade-in slide-in-from-top-2 duration-300">
                    <div className="p-6 border-b border-white/5 bg-white/5 flex items-center justify-between">
                        <div>
                            <h3 className="text-lg font-black text-white font-display uppercase tracking-tight">Intelligence</h3>
                        ...
                        {loading && notifications.length === 0 ? (
                            ...
                            <div className="divide-y divide-white/5">
                                {notifications.map(notification => (
                                    <div
                                        key={notification.id}
                                        className={`p-5 transition-all hover:bg-white/[0.03] group relative cursor-pointer ${!notification.is_read ? 'bg-indigo-500/[0.02]' : 'opacity-60'}`}
                                        onClick={() => {
                                    ...
                                    >
                                        ...
                                        <div className="flex items-start gap-4">
                                            <div className={`mt-0.5 w-10 h-10 rounded-2xl flex items-center justify-center border border-white/5 ${notification.type === 'error' ? 'bg-rose-500/10' :
                                            ...
                                            <div className="flex-1 min-w-0">
                                                <div className="flex justify-between items-start mb-1">
                                                    <h4 className="text-xs font-black text-white truncate pr-2">{notification.title}</h4>
                                                    ...
                                                    <div className="flex items-center gap-2">
                                                        {!notification.is_read && (
                                                            <button
                                                                ...
                                                                className="p-1 px-2.5 bg-indigo-500/10 hover:bg-indigo-500 text-indigo-400 hover:text-white rounded-lg text-[9px] font-black uppercase tracking-widest transition-all"
                                                            >
                                                                Resolve
                                                            </button>
                                                        )}
